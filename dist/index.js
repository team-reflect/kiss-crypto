...
import { arrayBufferToBase64, arrayBufferToString, base64ToArrayBuffer, generateRandomKey, } from './utils.js';
 ...
 */
export const encryptStringAsBlob = ({ key, plaintext, }) => {
 ...
 */
export function encryptBlob({ key, plainblob, }) {
    const nonce = generateRandomKey(Defaults.EncryptionNonceLength);
 ...
 */
export function decrypt({ key, ciphertext: encryptedMessage, }) {
 ...
 */
export const decryptBlobAsString = async ({ key, cipherblob, }) => {
 ...
 */
export function decryptBlob({ key, cipherblob: encryptedMessage, }) {
    const versionBuffer = encryptedMessage.slice(0, Defaults.Version.length);
    const version = arrayBufferToString(versionBuffer);
    if (version < Defaults.Version) {
        throw new Error(`Invalid version: ${version}`);
    }
    const nonceBuffer = encryptedMessage.slice(Defaults.Version.length, Defaults.Version.length + Defaults.EncryptionNonceLength);
    const cipherblob = encryptedMessage.slice(Defaults.Version.length + Defaults.EncryptionNonceLength);
 ...
 */
export function hash({ key, salt, length = 32, }) {
    const result = hkdf(sha256, utf8ToBytes(key), utf8ToBytes(salt), undefined, length);
    return bytesToHex(result);
}
// Private functions
function xChaChaEncrypt({ key, plaintext, nonce, }) {
    if (nonce.length !== 48)
        throw Error('Nonce must be 24 bytes');
    const plainblob = new TextEncoder().encode(plaintext);
    const arrayBuffer = xchacha20poly1305(hexToBytes(key), hexToBytes(nonce)).encrypt(plainblob);
    return arrayBufferToBase64(arrayBuffer);
}
function xChaChaEncryptBlob({ key, plainblob, nonceBuffer, }) {
...
function xChaChaDecrypt({ key, ciphertext, nonce, }) {
    const arr = xchacha20poly1305(hexToBytes(key), hexToBytes(nonce)).decrypt(base64ToArrayBuffer(ciphertext));
    return new TextDecoder().decode(arr);
}
function xChaChaDecryptBlob({ key, cipherblob, nonceBuffer, }) {
